...
CLI. `tools/builders.py` has PEP 723 inline metadata, with `dependencies = []`;
the implementation uses the standard library. The `./builders` wrapper, tmux
...
```bash
./builders init --codex-bin /absolute/path/to/codex
```
...
| Path | Purpose |
| --- | --- |
| `builders` | Convenient uv launcher |
| `tools/builders.py` | CLI, hooks, supervisor, scanner, and inbox |
| `AGENTS.md` | Canonical project instructions, maintained by the director |
...
replaced. Workers re-read it at assignment boundaries and after compaction.
...
`./builders monitor` can create it ahead of time. The existing director agent
...
./builders attach api-health
./builders steer api-health --prompt 'Keep the response schema unchanged; add the timeout case to validation.'
```
...
```bash
./builders report api-health --generation 1 --status done \
...
./builders inbox
./builders status
./builders inspect api-health
./builders diff api-health
./builders ack 42
```
...
`./builders watch` continuously prints JSONL events and scans panes. `scan` runs
...
Run `./builders monitor` again to recover a stopped or missing watcher; it does
...
./builders inspect api-health
./builders diff api-health
./builders merge api-health
# Run the appropriate combined project checks here.
./builders assign api-health --prompt 'Next narrow task with the same role.'
```
...
```bash
./builders checkpoint api-health -m 'Implement the reviewed health endpoint'
./builders steer api-health --prompt 'Your reviewed changes are committed. Verify HEAD and report done.'
```
...
```bash
./builders stop api-health
./builders restart api-health
```
...
```bash
uv run --cache-dir .builders/uv-cache --no-project python -m unittest discover -s tests -v
```
...
```bash
./builders --root /tmp/my-builder-experiment init \
  --session crafty-builders-experiment --socket crafty-builders-experiment
./builders --root /tmp/my-builder-experiment monitor
```
