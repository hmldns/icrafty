...
def install_cad(app, service, scoped):
    ...
    async def body(request):
        ...
        if not isinstance(value, dict) or not set(value) <= {"arguments", "bundle"} or "arguments" not in value:
    ...
    async def conversation(request: Request, sid: str, tool: str):
        mutate = tool in {"request_part", "request_evidence", "restore_geometry", "cancel"}
    ...
    async def model(request: Request, sid: str, tool: str):
        agent, operation = model_scope(request, sid, mutate=tool in {"cad_ensure", "cad_evaluate", "cad_release"})
        oid, generation = operation["id"], agent.generation
        value = await body(request)
        agent, current = model_scope(request, sid, mutate=tool in {"cad_ensure", "cad_evaluate", "cad_release"})
