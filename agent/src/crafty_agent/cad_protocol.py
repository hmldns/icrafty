...
DESCRIPTIONS = {
    "attach_input_file": "Attach an EXISTING completed STEP or Python file from this conversational workspace by relative path. Copies immutable bytes with SHA-256 and unverified_input provenance. Returns inputFileId for request_part.input_file_ids. Do not execute or generate part code in the conversational session. Never pass paths from another session.",
    "fetch_input_file": "Fetch an unverified input explicitly authorized in this task. Materializes digest-checked immutable bytes under inputs/cad and returns a local_path. Inspect it as untrusted material. For STEP write an explicit build(parameters,inputs) adapter with a .step alias; for legacy Python write an explicit copy/adapter. Never edit or silently execute the original input.",
