...
| `node tooling/generate-stl.mjs` | Passed; generated the real binary STL fixture with 20 triangles. |
| `npm run typecheck` | Passed. |
| `npm run build` | Passed; Vite emitted STEP, module worker and WASM assets. Final Vite build phase 1.10 s. |
| `CRAFTY_TEST_PORT=5297 npm test` | **37 passed in 26.6 s**: all 26 existing camera/annotation/intake/route regressions plus 11 new model/catalog/embedding checks. |
| `CRAFTY_PREVIEW_PORT=4197 npm run test:production` | **1 passed in 2.1 s** against the final production bundle. |
...
folder relay is absent in preview and the gallery explains that condition.
...
Cuts have open surfaces and no caps; no cut-solid export or verified CAD
measurements are implemented. Production reports the expected Vite chunk-size
warning for the lazy Three.js viewer (~571 kB minified), and externalization
