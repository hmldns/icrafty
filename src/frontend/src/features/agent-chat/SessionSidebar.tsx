...
export function SessionSidebar({ sessions, selected, creating, onCreate, onSelect, onRefresh }: {
  sessions: AgentSession[]; selected: string | null; creating: boolean;
  onCreate: () => void; onSelect: (id: string) => void; onRefresh: () => void;
}) {
  return <aside className="agent-sidebar" aria-label="Chats">
    <div className="agent-sidebar-heading"><h2>Your chats</h2><Button size="small" variant="ghost" onClick={onRefresh}>Refresh</Button></div>
    <Button variant="primary" icon="plus" disabled={creating} onClick={onCreate}>{creating ? "Starting Codex…" : "New chat"}</Button>
    <ul className="agent-session-list">
      {sessions.map(session => <li key={session.id}>
        <button type="button" className={cx("agent-session", selected === session.id && "agent-session--selected")}
          aria-pressed={selected === session.id} onClick={() => onSelect(session.id)}>
    ...
    <p className="agent-sidebar-note">Each chat keeps its own conversation and images on this machine.</p>
  </aside>;
}
